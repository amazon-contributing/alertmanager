...
func run() int {
	...
	groupFn := func(ctx context.Context, routeFilter func(*dispatch.Route) bool, alertFilter func(*types.Alert, time.Time) bool, groupIdsFilter func(string) bool) (dispatch.AlertGroups, map[model.Fingerprint][]string, error) {
		return disp.Groups(ctx, routeFilter, alertFilter, groupIdsFilter)
	}
