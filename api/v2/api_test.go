...
import (
	...
	"github.com/amazon-contributing/alertmanager/types"
...
func TestGetAlertGroupsHandler(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			api := API{
				uptime: time.Now(),
				alertGroups: func(context.Context, func(*dispatch.Route) bool, func(*types.Alert, time.Time) bool) (dispatch.AlertGroups, map[model.Fingerprint][]string, error) {
