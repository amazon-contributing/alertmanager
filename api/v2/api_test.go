	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			api := API{
				...
				alertmanagerConfig: &config.Config{},
				setAlertStatus:     newSetAlertStatus(alertsProvider),
			}
...
func TestGetAlertGroupsHandler(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			api := API{
				uptime: time.Now(),
				alertGroups: func(ctx context.Context, f func(*dispatch.Route) bool, f2 func(*alert.Alert, time.Time) bool, f3 func(string) bool) (dispatch.AlertGroups, map[model.Fingerprint][]string, error) {
					return aginfos, nil, nil
				},
				logger:             promslog.NewNopLogger(),
				apiCallback:        tc.callback,
				alertmanagerConfig: &config.Config{},
				...
				setAlertStatus: func(ctx context.Context, labels model.LabelSet) {},
			}
			r, err := http.NewRequest("GET", "/api/v2/alertgroups", nil)
			require.NoError(t, err)
...
func TestListAlertInfosHandler(t *testing.T) {
	...
	ags := dispatch.AlertGroups{
		&dispatch.AlertGroup{
			...
			Receiver: "testing",
			Alerts:   []*alert.Alert{alerts[0], alerts[1]},
		},
		&dispatch.AlertGroup{
			Labels: model.LabelSet{
				"alertname": "HighErrorRate",
			...
			Receiver: "prod",
			Alerts:   []*alert.Alert{alerts[2], alerts[3]},
		},
	}
...
func TestPostAlertHandler(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		api := API{
			uptime: time.Now(),
			alerts: newFakeAlerts([]*alert.Alert{}),
			logger: promslog.NewNopLogger(),
		...
		amAlert := OpenAPIAlertsToAlerts(context.Background(), alerts)
		if tc.code == 200 {
