...
func TestGetAlertGroupsHandler(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			api := API{
				uptime: time.Now(),
				alertGroups: func(ctx context.Context, f func(*dispatch.Route) bool, f2 func(*types.Alert, time.Time) bool, f3 func(string) bool) (dispatch.AlertGroups, map[model.Fingerprint][]string, error) {
					return aginfos, nil, nil
				},
				getAlertStatus: getAlertStatus,
				logger:         promslog.NewNopLogger(),
				apiCallback:    tc.callback,
				groupMutedFunc: func(routeID, groupKey string) ([]string, bool) { // ADD THIS
					return nil, false
				},
			}
			r, err := http.NewRequest("GET", "/api/v2/alertgroups", nil)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			api := API{
				...
				setAlertStatus: func(ctx context.Context, m model.LabelSet) {},
			}
			api.route = dispatch.NewRoute(&config.Route{Receiver: "def-receiver"}, nil)
