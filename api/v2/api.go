...
type (
	groupsFn         func(context.Context, func(*dispatch.Route) bool, func(*types.Alert, time.Time) bool, func(string) bool) (dispatch.AlertGroups, map[prometheus_model.Fingerprint][]string, error)
...
func (api *API) getAlertsFromAlertGroup(ctx context.Context, receiverFilter *regexp.Regexp, alertFilter func(a *types.Alert, now time.Time) bool, groupIdsFilter func(groupId string) bool) (open_api_models.GettableAlerts, error) {
	...
	alertGroups, allReceivers, err := api.alertGroups(ctx, routeFilter, alertFilter, groupIdsFilter)
	if err != nil {
		return nil, err
	}
	for _, alertGroup := range alertGroups {
...
func (api *API) getAlerts(ctx context.Context, receiverFilter *regexp.Regexp, alertFilter func(a *types.Alert, now time.Time) bool) (open_api_models.GettableAlerts, error) {
	...
	for a := range alerts.Next() {
		...
		routes := api.route.Match(a.Data.Labels)
		...
		alert := AlertToOpenAPIAlert(a.Data, api.getAlertStatus(a.Data.Fingerprint()), receivers, nil)
